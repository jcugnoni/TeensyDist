...
void AudioEffectWaveshaperF::setTanh(float b[5]) {
    ...
    for(int i=0; i<DISTTABLESIZE; i++) {
      ...
      this->wavetable[i]=DISTTABLEBND * _sign(y)*min(1.0,fabs(y));
    }
    #ifdef DEBUGDIST
...
void AudioEffectWaveshaperF::setExp(float b[4]) {
    ...
    for(int i=0; i<DISTTABLESIZE; i++) {
      ...
      this->wavex[i]=x1;
      float xp=x+coefs[2]; 
      float y=coefs[0]*_sign(xp)*(1-expf(coefs[1]*fabs(xp)))+coefs[3];
      this->wavetable[i]=(DISTTABLEBND * (_sign(y)*min(1.0,fabs(y))));
    }
    #ifdef DEBUGDIST
    Serial.println("Setting waveshaperf table:");
...
void AudioEffectWaveshaperF::setPow(float b[5]) {
    ...
    for(int i=0; i<DISTTABLESIZE; i++) {
      float x=((float)(-DISTTABLEBND+i*DISTTABLESTEP))/( (float) DISTTABLEBND);
      this->wavex[i]=(DISTTABLEBND*x);
      float y=b[0] * _sign(b[1] * x+ b[2]) * powf( fabs(b[1] * x+ b[2]), b[3]) + b[4];
      this->wavetable[i]=(DISTTABLEBND * (_sign(y)*min(1.0,fabs(y))));
    }
    #ifdef DEBUGDIST
    Serial.println("Setting waveshaperf table:");
...
void AudioEffectWaveshaperF::setCubic(float b[4]) {
    ...
    for(int i=0; i<DISTTABLESIZE; i++) {
      ...
      float x2=xb*xb;
      float y=(coefs[0]*(fabs(2*xb)-x2)*_sign(xb)+coefs[3]); //y=postgain*((abs(2*pregain*(x+bias))-(pregain*(x+bias))^2)*sign(x+bias))+offset   wavetable[i]=(DISTTABLEBND * (sign(y)*min(1.0,abs(y))));
      this->wavex[i]=(DISTTABLEBND*x);
      this->wavetable[i]=(DISTTABLEBND * (_sign(y)*min(1.0,fabs(y))));
    }
    #ifdef DEBUGDIST
...
void AudioEffectWaveshaperF::setAsym(float b[4]) {
    ...
    for(int i=0; i<DISTTABLESIZE; i++) {
      ...
      this->wavex[i]=(DISTTABLEBND*x);
      this->wavetable[i]=(DISTTABLEBND * (_sign(y)*min(1.0,fabs(coefs[0]*y))));
    }
    #ifdef DEBUGDIST
    Serial.println("Setting waveshaperf table:");
    for(int i=0; i<DISTTABLESIZE; i++) {
...
void AudioEffectWaveshaperF::setChebyshev(float b[6]) {
    ...
    for(int i=0; i<DISTTABLESIZE; i++) {
      ...
      wavetable[i]=(DISTTABLEBND * (_sign(y)*min(1.0,fabs(y))));
      this->wavex[i]=(DISTTABLEBND*x);
    }
    #ifdef DEBUGDIST
    Serial.println("Setting waveshaperf table:");
